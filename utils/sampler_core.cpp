...
class TemporalGraphBlock
{
    public:
        ...
        TemporalGraphBlock(std::vector<NodeIDType> &_row, std::vector<NodeIDType> &_col,
                           ...
                           NodeIDType _dim_in, NodeIDType _dim_out, std::vector<NodeIDType> &_degrees) :
                           row(_row), col(_col), eid(_eid), ts(_ts), dts(_dts),
                           nodes(_nodes), dim_in(_dim_in), dim_out(_dim_out), degrees(_degrees) {}
};
...
#pragma omp parallel for schedule(static, int(ceil(static_cast<float>(root_nodes.size()) / num_threads)))
        ...
        inline void combine_coo(TemporalGraphBlock &_ret, std::vector<NodeIDType> **_row, 
                                ...
                                std::vector<NodeIDType> **_degrees)
        {
            ...
            _ret.degrees.resize(cum_col.back());
...
#pragma omp parallel for schedule(static, 1)
            for (int tid = 0; tid < num_threads; tid++)
            {
                ...
                std::copy(_degrees[tid]->begin(), _degrees[tid]->end(), _ret.degrees.begin() + cum_col[tid]);
                          ...
                          int neighs, bool use_ptr, bool from_root)
        {
            ...
            for (int i = 0; i < num_history; i++)
            {
                ...
                std::vector<NodeIDType> *_degrees[num_threads];
...
#pragma omp parallel
                {
                    ...
                    _degrees[tid] = new std::vector<NodeIDType>;
                    ...
                    _degrees[tid]->reserve(reserve_capacity);
// #pragma omp critical
...
#pragma omp for schedule(static, int(ceil(static_cast<float>((*root_nodes).size()) / num_threads)))
                    for (std::vector<NodeIDType>::size_type j = 0; j < (*root_nodes).size(); j++)
                    {
                        ...
                        if ((recent) || (e_search - indptr[n] < neighs))
                        {                            
                            ...
                            //     }
                            // }
                            EdgeIDType trueCount = 0;
                            for (EdgeIDType k = e_search; k >std::max(indptr[n], 0); k--)
                            {
                                if (ts[k] < nts + offset - 1e-7f)
                                {
                                    add_neighbor(_row[tid], _col[tid], _eid[tid], _ts[tid], 
                        ...
                        for (EdgeIDType l = e_search; l >= std::max(s_search, 0); l--)
                        {
                            if (ts[l] <= nts)
                            {
                               _degrees[tid]->push_back(l - s_search);
                               break;
                            }
                        }
                ...
                ret[ret.size() - 1 - i].degrees.resize(root_nodes->size());
                combine_coo(ret[ret.size() - 1 - i], _row, _col, _eid, _ts, _dts, _nodes, _out_node, _degrees);
...
PYBIND11_MODULE(sampler_core, m)
{
    py::class_<TemporalGraphBlock>(m, "TemporalGraphBlock")
        .def(py::init<std::vector<NodeIDType> &, std::vector<NodeIDType> &,
                      ...
                      NodeIDType, NodeIDType, std::vector<NodeIDType> &>())
        ...
        .def("degrees", [](const TemporalGraphBlock &tgb) { return vec2npy(tgb.degrees); })
