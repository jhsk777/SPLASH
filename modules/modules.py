...
class GeneralModel(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, sample_param, memory_param, gnn_param, train_param, combined=False, mask=False, device='cuda:0'):
        ...
        if gnn_param['arch'] == 'transformer_attention':
            for h in range(sample_param['history']):
                self.layers['l0h' + str(h)] = TransfomerAttentionLayer(self.dim_node_input, dim_edge, gnn_param['dim_time'], gnn_param['att_head'], train_param['dropout'], train_param['att_dropout'], gnn_param['dim_out'], combined=combined, device=device)
            for l in range(1, gnn_param['layer']):
                for h in range(sample_param['history']):
                    self.layers['l' + str(l) + 'h' + str(h)] = TransfomerAttentionLayer(gnn_param['dim_out'], dim_edge, gnn_param['dim_time'], gnn_param['att_head'], train_param['dropout'], train_param['att_dropout'], gnn_param['dim_out'], combined=False, device=device)
        ...
        elif gnn_param['arch'] == 'SLIM':
            for h in range(sample_param['history']):
                self.layers['l0h' + str(h)] = SLIM(self.dim_node_input, dim_edge, gnn_param['dim_time'], train_param['dropout'], gnn_param['dim_out'], skip_alpha=gnn_param['skip_alpha'], device=device)
            for l in range(1, gnn_param['layer']):
                for h in range(sample_param['history']):
                    self.layers['l' + str(l) + 'h' + str(h)] = SLIM(gnn_param['dim_out'], dim_edge, gnn_param['dim_time'], train_param['dropout'],gnn_param['dim_out'], skip_alpha=gnn_param['skip_alpha'], device=device)   
        else:
            raise NotImplementedError
