import torch
import dgl
from modules.layers import TimeEncode
...
class MailBox():
    ...
    def __init__(self, memory_param, num_nodes, dim_edge_feat, _node_memory=None, _node_memory_ts=None,_mailbox=None, _mailbox_ts=None, _next_mail_pos=None, _update_mail_pos=None, device='cpu'):
        ...
        self.recent_rep = torch.zeros((num_nodes, memory_param['dim_out']), dtype=torch.float32)
        self.uniform_rep = torch.zeros((num_nodes, memory_param['dim_out']), dtype=torch.float32)
    ...
    def reset(self):
        ...
        self.recent_rep.fill_(0)
        self.uniform_rep.fill_(0)

    def move_to_gpu(self, device='cuda:0'):
        ...
        self.recent_rep = self.recent_rep.cuda()
        self.uniform_rep = self.uniform_rep.cuda()       
        self.device = torch.device(device)
    ...
    def prep_input_mails_aug(self, mfg, use_pinned_buffers=False, aug_ratio=0.1):
        for i, b in enumerate(mfg):
            b.srcdata['mem'] = self.node_memory[b.srcdata['ID'].long()].cuda()
            memory_sign = torch.sign(b.srcdata['mem'])
            noise = torch.rand_like(b.srcdata['mem']) * aug_ratio
            noise = noise * memory_sign
            b.srcdata['mem'] += noise.cuda()
            b.srcdata['mem_ts'] = self.node_memory_ts[b.srcdata['ID'].long()].cuda()
            b.srcdata['mem_input'] = self.mailbox[b.srcdata['ID'].long()].cuda().reshape(b.srcdata['ID'].shape[0], -1)
            b.srcdata['mail_ts'] = self.mailbox_ts[b.srcdata['ID'].long()].cuda()
    ...
    def random_update_memory(self, nid, memory, root_nodes, ts, neg_samples=1, aug_ratio=0.1):
        if nid is None:
            return
        num_true_src_dst = root_nodes.shape[0] // (neg_samples + 2) * 2
        with torch.no_grad():
            nid = nid[:num_true_src_dst].to(self.device)
            num_samples = int(nid.shape[0] * (1-aug_ratio))
            indices = torch.randperm(nid.shape[0])[:num_samples]
            random_nid = nid[indices]
            memory = memory[:num_true_src_dst].to(self.device)
            ts = ts[:num_true_src_dst].to(self.device)
            self.node_memory[random_nid.long()] = memory[indices]
            self.node_memory_ts[random_nid.long()] = ts[indices]

    def update_mailbox(self, nid, memory, root_nodes, ts, edge_feats, block, neg_samples=1):
        with torch.no_grad():
            num_true_edges = root_nodes.shape[0] // (2 + neg_samples)
            memory = memory.to(self.device)
            if edge_feats is not None:
                edge_feats = edge_feats.to(self.device).to(torch.float32)
            ...
            if self.memory_param['deliver_to'] == 'self':
                ...
                nid = nid[perm].to(self.mailbox.device)
                mail = mail[perm]
